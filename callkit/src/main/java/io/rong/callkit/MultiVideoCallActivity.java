...
public class MultiVideoCallActivity extends BaseCallActivity {
    ...
    public void onRemoteUserLeft(String userId, RongCallCommon.CallDisconnectedReason reason) {
        ...
        if (participantPortraitContainer != null) {
            ...
            Log.e(TAG, "onRemoteUserLeft->>participantPortraitContainer is empty");
        }

        if (isTopContainerUserExit(userId)) {
            Log.e(TAG, "onRemoteUserLeft->>isTopContainerUserExit---true");
            return;
        }
        ...
        if (localViewUserId == null) {
            Log.e(TAG, "onRemoteUserLeft->>localViewUserId is empty");
            return;
        }
        if (localViewUserId.equals(userId)) {
            Log.d(TAG, "onRemoteUserLeft->>localViewUserId---localViewUserId: " + localViewUserId);
        ...
        if (remoteViewContainer2 != null && !TextUtils.isEmpty(delUserid)) { // 删除退出用户的头像框
            Log.d(TAG, "onRemoteUserLeft->>remoteViewContainer2!=null");
            View singleRemoteView =
                    remoteViewContainer2.findViewWithTag(
    ...
    public void onRemoteUserInvited(String userId, RongCallCommon.CallMediaType mediaType) {
        ...
        if (participantPortraitContainer != null
                && participantPortraitContainer.getVisibility() == View.VISIBLE) {
            Log.i(TAG, "onRemoteUserInvited-->participantPortraitContainer--userId: " + userId);
            try {
            ...
            Log.d(TAG, "onRemoteUserInvited-->addSingleRemoteView--userId: " + userId);
            for (CallUserProfile profile : callSession.getParticipantProfileList()) {
